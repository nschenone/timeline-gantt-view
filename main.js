...
function compareEntriesForLayout(a, b) {
...
var LayoutEngine = class {
   ...
   */
  layout(entries, axis, mode = "compact") {
   ...
   */
  layoutGrouped(groups, axis, mode = "compact") {
    this.rowYMap.clear();
    this.nextAbsoluteRow = 0;
   ...
   */
  assignRows(entries, axis, baseRow, mode) {
  ...
  computeEntryGeometry(entry, axis) {
    entry.x = axis.yearToPixel(entry.startYear);
    if (entry.isPoint) {
      entry.width = 0;
    } else {
...
// src/utils/viewConfigUtils.ts
function normalizeConfigToken(value) {
  var _a;
  return (_a = value == null ? void 0 : value.trim().toLowerCase()) != null ? _a : "";
}
function parseColorMapJson(raw) {
...
function parseTooltipPropsJson(raw) {
  if (!(raw == null ? void 0 : raw.trim())) return null;
  try {
    const parsed = JSON.parse(raw);
    ...
    const props = parsed.map((value) => normalizeConfigToken(String(value != null ? value : ""))).filter(Boolean);
...
var _BarRenderer = class _BarRenderer {
  constructor(svg, app, tooltipParent) {
    ...
    this.tooltipPropsAllowlist = null;
  ...
  setTooltipPropsAllowlist(props) {
    this.tooltipPropsAllowlist = (props == null ? void 0 : props.length) ? new Set(props) : null;
  }
  ...
  showTooltip(entry, e) {
    ...
    for (const [key, val] of Object.entries(entry.properties)) {
      ...
      const rawKey = normalizeConfigToken((_c = entry.propertyKeys[key]) != null ? _c : key);
      const allowlist = this.tooltipPropsAllowlist;
      const shouldInclude = !allowlist || allowlist.has(keyLower) || allowlist.has(rawKey);
      if (val && val !== "null" && shouldInclude && !_BarRenderer.TOOLTIP_EXCLUDE_BASE.has(keyLower) && !_BarRenderer.TOOLTIP_EXCLUDE_BASE.has(rawKey) && !this.tooltipExcludeExtra.has(keyLower) && !this.tooltipExcludeExtra.has(rawKey)) {
        this.tooltipEl.createEl("br");
        const span = this.tooltipEl.createEl("span", { cls: "sabidurian-tooltip-prop" });
...
function getColorForValue(value, overrides) {
  if (!value) return COLOR_VARS[0];
  const trimmed = value.trim();
  const normalized = normalizeConfigToken(trimmed);
  const override = overrides == null ? void 0 : overrides[normalized];
  if (override) {
    return override;
  }
...
var SabidurianView = class extends import_obsidian6.BasesView {
  ...
  onDataUpdated() {
    ...
    const colorOverrides = parseColorMapJson(this.config.get("colorMapJson"));
    const tooltipProps = parseTooltipPropsJson(this.config.get("tooltipPropsJson"));
  ...
  parseEntries(entries, startPropId, endPropId, colorPropId, depPropName, enableBCE = true, earliestStartPropId, latestEndPropId, colorOverrides) {
    var _a;
    const result = [];
    for (const entry of entries) {
  ...
  getStackingMode() {
    return this.config.get("stackingMode") === "one-row-per-bar" ? "one-row-per-bar" : "compact";
  }
  /**
...
var SequenceView = class extends import_obsidian7.BasesView {
  ...
  parseSequenceEntries(entries, orderPropId, orderEndPropId, colorPropId, depPropName, colorOverrides) {
    const result = [];
    for (const entry of entries) {
...
function getViewOptions(config) {
  ...
  return [
    ...
    },
    {
      type: "dropdown",
      key: "stackingMode",
      displayName: "Stacking mode",
      default: "compact",
      options: {
        compact: "Compact",
        "one-row-per-bar": "One row per bar"
      }
    },
    // ── Focus filter ──
    {
